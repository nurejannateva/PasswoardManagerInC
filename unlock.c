...
static void on_save_clicked(GtkButton *btn, gpointer data) {
    ...
    FormData *f = (FormData *)data;

    const char *user_text = gtk_editable_get_text(GTK_EDITABLE(f->username));
    const char *pass_text = gtk_editable_get_text(GTK_EDITABLE(f->password));
    ...
    GtkWidget *user = gtk_label_new(user_text);
    GtkWidget *pass = gtk_label_new(pass_text);
    ...
    gtk_box_append(GTK_BOX(f->list), row);

    gtk_window_destroy(GTK_WINDOW(f->dialog));
    g_free(f);
}
...
static void on_add_clicked(GtkButton *btn, gpointer user_data) {
    ...
    GtkWidget *list = g_object_get_data(G_OBJECT(window), "list");
    ...
    FormData *f = g_malloc(sizeof(FormData));
    f->list = list;
    f->platform = platform;
    f->username = username;
    f->password = password;
    ...
    g_signal_connect(save, "clicked", G_CALLBACK(on_save_clicked), f);
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    const char *css =
        ...
        "#card { background: #d9fbf3; border-radius: 30px; padding: 25px; }"

        "#btn { background: #0f9d8f; color: white; border-radius: 14px; padding: 10px 22px; }"

        "#row { background: #f4fffd; padding: 18px; border-radius: 18px; }"

        "#icon { background: white; border-radius: 14px; padding: 10px; }"

        "#danger { background: #ffecec; border-radius: 14px; padding: 10px; }"

        "#modal-bg { background: rgba(0,0,0,0.15); }"

        "#modal-card { background: #f4fffd; border-radius: 25px; padding: 25px; }"

        "#modal-title { font-size: 22px; font-weight: bold; }"

        "entry { background: #eafff9; border-radius: 20px; padding: 14px; }"

        "#modal-save { background: #0f9d8f; color: white; border-radius: 14px; padding: 12px; }"

        "#modal-cancel { background: transparent; color: #0f9d8f; }";
